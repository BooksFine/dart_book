...
- **Поддержка форматов:** чтение и запись EPUB 2/3, FB2 и архивов FB2.zip.
- **Единая модель данных:** иерархия узлов на базе Dart 3 `sealed class` (`BookBlock`, `BookInline`, `BookMetadata`, `BookResource`).
- **Фоновые изоляты:** асинхронный разбор и кодирование в `Isolate.run` (`DartBook.loadIsolated`, `DartBook.encodeIsolated`).
- **Сквозная конвертация:** трансляция между EPUB и FB2 с сохранением структуры, сносок, стилей и ресурсов.
- **Обработка повреждённых данных:** санитизация некорректных символов XML 1.0, незаэкранированных амперсандов, декодирование HTML-сущностей, автоматическое определение кодировок UTF-8 и Windows-1251.
- **Безопасность архивов:** нормализация путей и защита от path traversal (Zip Slip) при распаковке в памяти, ограничение глубины навигации.
- **Шрифты и DRM:** автоматическая деобфускация шрифтов IDPF (SHA-1) и Adobe (UUID XOR), детекция зашифрованных DRM-ресурсов.
- **Сложный контент:** разбор таблиц (`colspan`, `rowspan`, выравнивание), блоков кода, стихотворных строф, цитат, сносок, формул MathML, векторного SVG, аудио и видео.
- **Синхронизация звука (SMIL 3.0):** парсер медиа-наложений Media Overlays с поддержкой временных меток (`hh:mm:ss`, `ms`, `s`, `min`, `h`).

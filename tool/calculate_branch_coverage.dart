...
void main() {
  ...
  print(
    '                    АНАЛИЗ ПОКРЫТИЯ ВЕТВЛЕНИЙ (BRANCH COVERAGE)                 ',
  );
  print(
    '════════════════════════════════════════════════════════════════════════════════\n',
  );
  ...
  if (!hasBranchData) {
    print(
      'Файлы профилирования не содержат сырых branchHits (использовался стандартный формат).',
    );
    return;
  }
  ...
  for (final source in sortedSources) {
    ...
    final percent = total > 0
        ? (hit / total * 100).toStringAsFixed(1)
        : '100.0';
    ...
    final paddedName = shortName.length > 48
        ? '${shortName.substring(0, 45)}...'
        : shortName.padRight(48);
    print(
      '$paddedName ${total.toString().padLeft(6)}     ${hit.toString().padLeft(6)}   ${percent.padLeft(5)}%',
    );
  }
  ...
  final totalPercent = totalBranches > 0
      ? (hitBranches / totalBranches * 100).toStringAsFixed(2)
      : '0.00';
  final handPercent = handTotalBranches > 0
      ? (handHitBranches / handTotalBranches * 100).toStringAsFixed(2)
      : '0.00';
  print(
    'ИТОГО BRANCH COVERAGE (ВСЕ ФАЙЛЫ):                  ${totalBranches.toString().padLeft(6)}     ${hitBranches.toString().padLeft(6)}   ${totalPercent.padLeft(5)}%',
  );
  print(
    'ИТОГО BRANCH COVERAGE (РУКОПИСНЫЙ КОД):              ${handTotalBranches.toString().padLeft(6)}     ${handHitBranches.toString().padLeft(6)}   ${handPercent.padLeft(5)}%',
  );
  print(
    '\n════════════════════════════════════════════════════════════════════════════════',
  );

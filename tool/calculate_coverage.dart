...
void main() {
  ...
  if (!coverageDir.existsSync()) {
    print(
      'Coverage directory not found. Run "dart test --coverage=coverage" first.',
    );
    exit(1);
  }
  ...
  print(
    '                    ОТЧЕТ О ПОКРЫТИИ ТЕСТАМИ (CODE COVERAGE)                    ',
  );
  print(
    '════════════════════════════════════════════════════════════════════════════════\n',
  );
  ...
  for (final source in sortedSources) {
    ...
    final paddedName = shortName.length > 50
        ? '${shortName.substring(0, 47)}...'
        : shortName.padRight(50);
    ...
    // Grouping
    final groupKey = shortName.contains('/')
  ...
  final totalPercent = totalLines > 0
      ? (hitLines / totalLines * 100).toStringAsFixed(2)
      : '0.00';
  print(
    'ИТОГО ПО ВСЕЙ БИБЛИОТЕКЕ:                            ${totalLines.toString().padLeft(6)}     ${hitLines.toString().padLeft(6)}   ${totalPercent.padLeft(5)}%\n',
  );

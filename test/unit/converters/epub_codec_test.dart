  ...
  group('EPUB Codec Unit Tests (EpubDecoder, EpubEncoder, OcfContainer, Nav/Ncx)', () {
    ...
    group('OCF Container: uncompressed mimetype and META-INF/container.xml', () {
      test('EpubEncoder writes uncompressed mimetype at offset 0 (0..58 bytes)', () {
        final book = Book(
          metadata: const BookMetadata(id: 'mimetype-test', title: 'Mimetype Test', language: 'en'),
          content: const BookContent(
            blocks: [BookParagraph(inlines: [BookText('Hello EPUB')])],
          ),
          resources: const [],
        );
      ...
      test('OcfContainer parses single and multiple rootfiles from container.xml', () {
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    ...
    group('Font Deobfuscation: IDPF (SHA-1) and Adobe (UUID XOR) algorithms', () {
      test('IDPF Font Deobfuscation: verifies OpenType (OTTO) font header reconstruction', () {
        ...
        expect(String.fromCharCodes(rawFontBytes.sublist(0, 4)), equals('OTTO'));
        ...
        expect(String.fromCharCodes(obfuscatedBytes.sublist(0, 4)), isNot(equals('OTTO')));
        ...
        expect(String.fromCharCodes(restoredBytes.sublist(0, 4)), equals('OTTO'));
        expect(restoredBytes, equals(rawFontBytes));
      });

      test('Adobe Font Deobfuscation: verifies TrueType (\\x00\\x01\\x00\\x00) font header reconstruction', () {
        const publicationUid = 'urn:uuid:48e24440-2b47-49f3-8b77-84883448f7a6';
        ...
        final hex = publicationUid.replaceAll('urn:uuid:', '').replaceAll('-', '').replaceAll(':', '');
        final key = <int>[];
        ...
        expect(obfuscatedBytes.sublist(0, 4), isNot(equals([0x00, 0x01, 0x00, 0x00])));
      ...
      test('Handles small font files (< 1024 / 1040 bytes) and empty bytes safely', () {
        const uid = 'urn:uuid:abcdef01-2345-6789-abcd-ef0123456789';

        // 1. Small 300-byte font
        final smallFont = Uint8List.fromList(List.generate(300, (i) => (i * 7) % 256));
        final obfIdpf = OcfContainer.deobfuscateFont(smallFont, 'http://www.idpf.org/2008/embedding', uid);
        final restIdpf = OcfContainer.deobfuscateFont(obfIdpf, 'http://www.idpf.org/2008/embedding', uid);
        expect(restIdpf, equals(smallFont));

        final obfAdobe = OcfContainer.deobfuscateFont(smallFont, 'http://ns.adobe.com/pdf/enc#RC', uid);
        final restAdobe = OcfContainer.deobfuscateFont(obfAdobe, 'http://ns.adobe.com/pdf/enc#RC', uid);
        expect(restAdobe, equals(smallFont));

        // 2. Empty byte array
        final emptyBytes = Uint8List(0);
        expect(OcfContainer.deobfuscateFont(emptyBytes, 'http://www.idpf.org/2008/embedding', uid), isEmpty);
      });

      test('End-to-end EpubDecoder font deobfuscation from archive with META-INF/encryption.xml', () async {
...
<package xmlns="http://www.idpf.org/2007/opf" unique-identifier="uid" version="3.0">
  ...
  <spine><itemref idref="ch1"/></spine>
        ...
        archive.addFile(ArchiveFile('OEBPS/fonts/font.otf', obfFont.length, obfFont));
        ...
        expect(fontRes.bytes.sublist(0, 4), equals([0x4F, 0x54, 0x54, 0x4F])); // 'OTTO'
    ...
    group('Dual Navigation (EPUB 3 nav.xhtml & EPUB 2 toc.ncx)', () {
      test('EpubEncoder simultaneously generates nav.xhtml (EPUB 3) and toc.ncx (EPUB 2)', () {
        final book = Book(
          metadata: const BookMetadata(id: 'dual-nav-book', title: 'Dual Nav Book', language: 'ru'),
          content: const BookContent(
            blocks: [
              BookSection(
                title: [BookText('Глава 1: Начало')],
                blocks: [BookParagraph(inlines: [BookText('Текст первой главы')])],
              ),
              BookSection(
                title: [BookText('Глава 2: Развитие')],
                blocks: [BookParagraph(inlines: [BookText('Текст второй главы')])],
              ),
            ],
          ),
          resources: const [],
        ...
        expect(navContent, contains('<a href="chapter_1.xhtml">Глава 1: Начало</a>'));
        expect(navContent, contains('<a href="chapter_2.xhtml">Глава 2: Развитие</a>'));
        ...
        expect(ncxContent, contains('<ncx xmlns="http://www.daisy.org/z3986/2005/ncx/"'));
        ...
        expect(opfContent, contains('<item id="nav" href="nav.xhtml" media-type="application/xhtml+xml" properties="nav"/>'));
        expect(opfContent, contains('<item id="ncx" href="toc.ncx" media-type="application/x-dtbncx+xml"/>'));
...
<ncx xmlns="http://www.daisy.org/z3986/2005/ncx/" version="2005-1">
  ...
  <navMap>
    ...
    // -------------------------------------------------------------------------
    group('OPF Metadata: belongs-to-collection, rendition:layout, ISBN, publisher, date', () {
      test('EpubEncoder & EpubDecoder handle EPUB 3 collections with group-position', () async {
        ...
        expect(opfContent, contains('<meta property="belongs-to-collection" id="series-1">Хроники Дюны</meta>'));
        expect(opfContent, contains('<meta refines="#series-1" property="group-position">3</meta>'));
        expect(opfContent, contains('<meta property="belongs-to-collection" id="series-2">Космическая фантастика</meta>'));
        expect(opfContent, contains('<meta refines="#series-2" property="group-position">12</meta>'));
        ...
        expect(decodedBook.metadata.series[1].name, equals('Космическая фантастика'));
        expect(decodedBook.metadata.series[1].number, equals(12));
      });

      test('EpubEncoder & EpubDecoder handle Rendition Layout (pre-paginated & roll)', () async {
        ...
        final fixedBook = Book(
          ...
          content: const BookContent(blocks: [BookParagraph(inlines: [BookText('Page 1')])]),
          resources: const [],
        );
        final fixedBytes = EpubEncoder().encode(fixedBook);
        final fixedArchive = ZipDecoder().decodeBytes(fixedBytes);
        final fixedOpf = utf8.decode(fixedArchive.findFile('OEBPS/content.opf')!.content);
        expect(fixedOpf, contains('<meta property="rendition:layout">pre-paginated</meta>'));

        final decodedFixed = await EpubDecoder().decode(fixedBytes);
        expect(decodedFixed.metadata.layout, equals(BookLayout.fixedLayout));
        ...
        final rollBook = Book(
          ...
          content: const BookContent(blocks: [BookParagraph(inlines: [BookText('Webtoon scroll')])]),
          resources: const [],
        );
        final rollBytes = EpubEncoder().encode(rollBook);
        final rollArchive = ZipDecoder().decodeBytes(rollBytes);
        final rollOpf = utf8.decode(rollArchive.findFile('OEBPS/content.opf')!.content);
        expect(rollOpf, contains('<meta property="rendition:layout">roll</meta>'));

        final decodedRoll = await EpubDecoder().decode(rollBytes);
        expect(decodedRoll.metadata.layout, equals(BookLayout.roll));
      });

      test('EpubEncoder & EpubDecoder handle ISBN URN, dc:publisher, dc:date, source-language, dc:source', () async {
        final originalBook = Book(
          metadata: BookMetadata(
            ...
            srcTitleInfo: const BookSourceTitleInfo(title: 'Das Originalbuch'),
          ),
          content: const BookContent(
            blocks: [
              BookSection(
                title: [BookText('Введение')],
                blocks: [BookParagraph(inlines: [BookText('Текст введения')])],
              ),
            ],
          ),
          resources: const [],
        ...
        // Verify XML tags
        expect(opfContent, contains('<dc:identifier id="pub-isbn">urn:isbn:978-5-00195-123-4</dc:identifier>'));
        expect(opfContent, contains('<dc:publisher>Манн, Иванов и Фербер</dc:publisher>'));
        expect(opfContent, contains('<dc:date>2023-10-25</dc:date>'));
        expect(opfContent, contains('<meta property="source-language">de</meta>'));
        expect(opfContent, contains('<dc:source>Das Originalbuch</dc:source>'));

        // Decode and verify properties
        final decodedBook = await EpubDecoder().decode(bytes);
        expect(decodedBook.metadata.publishInfo?.publisher, equals('Манн, Иванов и Фербер'));
        expect(decodedBook.metadata.publishInfo?.isbn, equals('978-5-00195-123-4'));
        expect(decodedBook.metadata.publishedAt, equals(DateTime(2023, 10, 25)));
        expect(decodedBook.metadata.srcLang, equals('de'));
        expect(decodedBook.metadata.srcTitleInfo?.title, equals('Das Originalbuch'));
      });
    });
    ...
    // -------------------------------------------------------------------------
    group('Exceptions: EpubEncryptedResourceException, EpubInvalidPackageException, Strict Mode', () {
      test('Throws EpubEncryptedResourceException on actual DRM encrypted content', () async {
        final archive = Archive();
        archive.addFile(ArchiveFile.noCompress('mimetype', 20, utf8.encode('application/epub+zip')));
        archive.addFile(
          ArchiveFile(
...
<package xmlns="http://www.idpf.org/2007/opf" unique-identifier="uid" version="3.0">
  ...
  <spine><itemref idref="ch1"/></spine>
      ...
      test('Throws EpubInvalidPackageException when META-INF/container.xml is missing or has no rootfiles', () {
        // 1. Missing container.xml
        final archiveWithoutContainer = Archive();
        archiveWithoutContainer.addFile(ArchiveFile.noCompress('mimetype', 20, utf8.encode('application/epub+zip')));
        final zip1 = Uint8List.fromList(ZipEncoder().encode(archiveWithoutContainer));
        ...
        archiveEmptyContainer.addFile(ArchiveFile.noCompress('mimetype', 20, utf8.encode('application/epub+zip')));
        archiveEmptyContainer.addFile(
          ArchiveFile(
            'META-INF/container.xml',
            100,
            utf8.encode('<?xml version="1.0"?><container version="1.0"><rootfiles></rootfiles></container>'),
          ),
        );
        final zip2 = Uint8List.fromList(ZipEncoder().encode(archiveEmptyContainer));
      ...
      test('Throws BookMalformedMetadataException in strictMode when dc:title is missing', () async {
        final archive = Archive();
        archive.addFile(ArchiveFile.noCompress('mimetype', 20, utf8.encode('application/epub+zip')));
        archive.addFile(
          ArchiveFile(
...
<package xmlns="http://www.idpf.org/2007/opf" unique-identifier="uid" version="3.0">
  ...
  <spine><itemref idref="ch1"/></spine>
        ...
        archive.addFile(ArchiveFile('OEBPS/chap1.xhtml', 50, utf8.encode('<html><body><p>Hi</p></body></html>')));

        final zipBytes = Uint8List.fromList(ZipEncoder().encode(archive));
        final decoder = EpubDecoder();

        expect(
          () => decoder.decode(zipBytes, options: const BookDecodingOptions(strictMode: true)),
    ...
    // -------------------------------------------------------------------------
    group('Comprehensive AST Roundtrip with GoldenComparator and AstNormalizer', () {
      test('Full roundtrip conversion of rich AST structures through EPUB', () async {
        final originalBook = Book(
          metadata: BookMetadata(
            id: 'epub-rich-rt-1',
            ...
            publishInfo: const BookPublishInfo(publisher: 'Классика', year: 2024),
          ),
          content: const BookContent(
            blocks: [
              BookSection(
                ...
                blocks: [
                  ...
                  BookQuote(
                    blocks: [BookParagraph(inlines: [BookText('Цитата классика')])],
                  ...
                  BookTable(
                    rows: [
                      BookTableRow(cells: [
                        BookTableCell(blocks: [BookParagraph(inlines: [BookText('Заголовок')])], colSpan: 2),
                      ]),
                      BookTableRow(cells: [
                        BookTableCell(blocks: [BookParagraph(inlines: [BookText('1.1')])]),
                        BookTableCell(blocks: [BookParagraph(inlines: [BookText('1.2')])]),
                      ]),
                    ],
                  ),
                  ...
                  BookAudioBlock(ref: BookResourceRef('sound.mp3'), controls: true),
                  BookVideoBlock(ref: BookResourceRef('video.mp4'), controls: true),
                  BookMathBlock(mathml: '<math><mi>x</mi></math>'),
                  BookSvgBlock(svg: '<svg><rect width="10" height="10"/></svg>'),
                  BookHorizontalRule(),
                ],
              ),
            ],
          ...
          resources: [
            BookResource(id: 'picture.png', mediaType: 'image/png', bytes: Uint8List.fromList([1, 2, 3])),
            BookResource(id: 'sound.mp3', mediaType: 'audio/mpeg', bytes: Uint8List.fromList([4, 5, 6])),
            BookResource(id: 'video.mp4', mediaType: 'video/mp4', bytes: Uint8List.fromList([7, 8, 9])),
          ],
        );
        ...
        expect(decodedBook.metadata.title, equals(originalBook.metadata.title));
        expect(decodedBook.metadata.language, equals(originalBook.metadata.language));
        expect(decodedBook.resources.length, equals(originalBook.resources.length));
        ...
        expect((section.title.first as BookText).text, equals('Глава 1: Начало'));
        ...
        expect((blocks[5] as BookCodeBlock).code, equals('void main() => print("EPUB");'));
        expect(blocks[6], isA<BookImageBlock>());
        expect((blocks[6] as BookImageBlock).ref.id, equals('epub-res-picture.png'));
        ...
        final textOnlyBook = Book(
          metadata: const BookMetadata(id: 'norm-epub-1', title: 'Norm Test', language: 'ru'),
          content: const BookContent(
            blocks: [
              BookSection(
                id: 'chapter_1',
                ...
                blocks: [
                  ...
                  BookQuote(
                    blocks: [BookParagraph(inlines: [BookText('Цитата')])],
        ...
        final decodedTextBook = await EpubConverter.epubToBook(textEpubBytes);
        final normDecoded = AstNormalizer.normalizeBook(decodedTextBook);
        final normOriginal = AstNormalizer.normalizeBook(textOnlyBook);
        GoldenComparator.assertContentEquals(normDecoded.content, normOriginal.content);
      });
    });
  });
}

    ...
    group('Windows-1251 encoding and decoding', () {
      test('Decodes complete Windows-1251 byte table (0x00 - 0xFF)', () {
        ...
        for (var b = 0x20; b <= 0xFF; b++) {
          ...
          if (b == 0x3C || b == 0x3E || b == 0x26 || b == 0x7F) continue; // '<', '>', '&', DEL
          byteList.add(b);
      ...
      test('Decodes specific Windows-1251 symbols: em-dash, quotes, №, ellipsis, Ukrainian/Belarusian letters', () {
        const phrase = '«№ 123 — Заголовок… Ґанок, Єдина Україна, Іван, Їжак, Ўзбекистан, ёлка, Ёжик»';
        final encodedBytes = _encodeWin1251('''<?xml version="1.0" encoding="windows-1251"?>
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  <description>
    <title-info>
  ...
  <body><p>$phrase</p></body>
      ...
      test('Decodes Windows-1251 with "cp1251" encoding declaration', () {
        ...
        final encodedBytes = _encodeWin1251('''<?xml version="1.0" encoding="cp1251"?>
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  <description>
  ...
  <body><p>$phrase</p></body>
      ...
      test('Auto-detects and decodes Windows-1251 even without explicit encoding header on non-UTF8 bytes', () {
        const phrase = 'Русский текст без явного объявления encoding';
        final encodedBytes = _encodeWin1251('''<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  <description>
...
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0" xmlns:l="http://www.w3.org/1999/xlink">
  ...
  <binary id="cover.jpg" content-type="image/jpeg">aW1hZ2UtYnl0ZXM=</binary>
      ...
      test('Fb2Decoder parses all metadata sections: title-info, src-title-info, publish-info, document-info, custom-info', () {
        final decoder = Fb2Decoder();
        final book = decoder.decode(Uint8List.fromList(utf8.encode(fullMetadataXml)));
        ...
        expect(book.metadata.genres.map((g) => g.code), containsAll(['sf_fantasy', 'cyberpunk']));
        expect(book.metadata.keywords, containsAll(['космос', 'океан', 'роботы']));
        ...
        // Authors
        final authors = book.metadata.contributorsByRole(BookContributorRole.author);
        ...
        // Translators
        final translators = book.metadata.contributorsByRole(BookContributorRole.translator);
        ...
        expect(book.metadata.series.first.url, equals(Uri.parse('https://lem.pl/series/diaries')));
      ...
      test('Fb2Encoder writes all metadata sections in strict schema compliance', () async {
        final decoder = Fb2Decoder();
        final encoder = const Fb2Encoder(programUsed: 'Test Program 1.0');

        final originalBook = decoder.decode(Uint8List.fromList(utf8.encode(fullMetadataXml)));
        ...
        expect(xmlOutput, contains('<book-title>Солярис и Кибериада</book-title>'));
        expect(xmlOutput, contains('<annotation>'));
        expect(xmlOutput, contains('<keywords>космос, океан, роботы, искусственный интеллект</keywords>'));
        ...
        expect(xmlOutput, contains('<sequence name="Звездные Дневники" number="7"/>'));
        ...
        expect(xmlOutput, contains('<program-used>Test Program 1.0</program-used>'));

        expect(xmlOutput, contains('<publish-info>'));
        expect(xmlOutput, contains('<publisher>Издательство Мир</publisher>'));
        ...
        expect(xmlOutput, contains('<custom-info info-type="sequence-url">https://lem.pl/series/diaries</custom-info>'));
        ...
        expect(redecoded.metadata.language, equals(originalBook.metadata.language));
        expect(redecoded.metadata.srcLang, equals(originalBook.metadata.srcLang));
        expect(redecoded.metadata.publishInfo?.publisher, equals(originalBook.metadata.publishInfo?.publisher));
        expect(redecoded.metadata.publishInfo?.isbn, equals(originalBook.metadata.publishInfo?.isbn));
        expect(redecoded.metadata.srcTitleInfo?.title, equals(originalBook.metadata.srcTitleInfo?.title));
        expect(redecoded.metadata.series.first.url, equals(originalBook.metadata.series.first.url));
      });
    });
    ...
    group('Body separation: main body vs notes and comments bodies', () {
      test('Separates <body name="notes"> and <body name="comments"> into content.footnotes', () {
  ...
  <body name="comments">
    <section id="comm1">
      ...
      <p>Текст авторского комментария.</p>
        ...
        final fn1Para = book.content.footnotes[0].blocks.first as BookParagraph;
        expect((fn1Para.inlines.first as BookText).text, equals('Текст сноски номер 1.'));

        final comm1Para = book.content.footnotes[1].blocks.first as BookParagraph;
        expect((comm1Para.inlines.first as BookText).text, equals('Текст авторского комментария.'));
      });

      test('Fb2Encoder generates <body name="notes"> for content.footnotes', () async {
        final book = Book(
          metadata: const BookMetadata(id: 'fn-test-1', title: 'Генерация сносок', language: 'ru'),
          content: const BookContent(
            blocks: [
              BookParagraph(inlines: [
                BookText('Текст с примечанием '),
                BookFootnoteRef(id: 'custom_note', label: [BookText('[Прим. ред.]')]),
              ]),
            ],
            footnotes: [
              BookFootnote(
                id: 'custom_note',
                blocks: [
                  BookParagraph(inlines: [BookText('Примечание редактора: важно!')]),
                ],
              ),
            ],
          ),
        ...
        final footnotePara = decoded.content.footnotes.first.blocks.first as BookParagraph;
        expect((footnotePara.inlines.first as BookText).text, equals('Примечание редактора: важно!'));
      });

      test('Does NOT generate <body name="notes"> when content.footnotes is empty', () async {
        final book = Book(
          metadata: const BookMetadata(id: 'no-fn-test', title: 'Без сносок', language: 'ru'),
          content: const BookContent(
            blocks: [BookParagraph(inlines: [BookText('Просто текст')])],
            footnotes: [],
          ),
          resources: const [],
        );
    ...
    group('Base64 Encoding & Decoding Resilience in <binary>', () {
      test('Decodes Base64 with line breaks (CRLF, LF), tabs, and internal whitespace', () {
        final originalData = Uint8List.fromList(List.generate(100, (i) => (i * 13) % 256));
  ...
  <binary id="img1.png" content-type="image/png">
    ${brokenBase64.toString()}
      ...
      test('Handles Base64 padding variants (=, ==, no padding needed)', () {
        final data1 = Uint8List.fromList([1, 2, 3]); // 3 bytes -> 4 chars (no pad)
...
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  ...
  <binary id="res3.bin" content-type="application/octet-stream">${base64Encode(data3)}</binary>
    ...
    // -------------------------------------------------------------------------
    group('FB2.ZIP packaging & unpacking (Fb2ZipDecoder, Fb2ZipEncoder, Fb2ZipConverter)', () {
      test('canDecode correctly identifies FB2.ZIP and rejects non-ZIP/EPUB files', () {
        final zipDecoder = Fb2ZipDecoder();

        // Valid zip bytes
        final validZip = Uint8List.fromList([0x50, 0x4B, 0x03, 0x04, ...List.filled(20, 0)]);
        expect(zipDecoder.canDecode(validZip, extension: 'fb2.zip'), isTrue);
        expect(zipDecoder.canDecode(validZip, extension: 'book.zip'), isTrue);
        ...
        final nonZip = Uint8List.fromList([0x3C, 0x3F, 0x78, 0x6D]); // '<?xm'
        expect(zipDecoder.canDecode(nonZip), isFalse);
      });

      test('Fb2ZipEncoder creates valid ZIP archive with configured options', () async {
        final book = Book(
          metadata: const BookMetadata(id: 'zip-opts-1', title: 'Опции ZIP архива', language: 'ru'),
          content: const BookContent(
            blocks: [BookParagraph(inlines: [BookText('Содержимое книги в ZIP')])],
          ),
          resources: const [],
        );
      ...
      test('Fb2ZipDecoder picks the .fb2/.xml file when archive contains multiple entries', () {
...
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  ...
  <body><p>Контент</p></body>
        ...
        archive.addFile(ArchiveFile('readme.txt', 12, utf8.encode('Read me info')));
        archive.addFile(ArchiveFile('book.fb2', fb2Xml.length, utf8.encode(fb2Xml)));
      ...
      test('Fb2ZipDecoder throws BookFormatException on corrupted/empty zip files', () {
        final zipDecoder = Fb2ZipDecoder();
        ...
        final emptyZipBytes = Uint8List.fromList(ZipEncoder().encode(emptyArchive));
        expect(
      ...
      test('Full roundtrip using Fb2ZipConverter with comprehensive AST node types', () async {
        final originalBook = Book(
          metadata: BookMetadata(
            ...
            publishInfo: const BookPublishInfo(publisher: 'Издатель', year: 2024),
          ),
          content: const BookContent(
            blocks: [
              BookSection(
                ...
                blocks: [
                  BookParagraph(inlines: [
                    ...
                    BookText(' и сноской '),
                    BookFootnoteRef(id: 'note-1', label: [BookText('[1]')]),
                    BookText('.'),
                  ]),
                  BookQuote(
                    blocks: [BookParagraph(inlines: [BookText('Цитата внутри раздела')])],
                  ...
                  BookTable(
                    rows: [
                      BookTableRow(cells: [
                        BookTableCell(blocks: [BookParagraph(inlines: [BookText('Ячейка 1.1')])], colSpan: 2),
                        BookTableCell(blocks: [BookParagraph(inlines: [BookText('Ячейка 1.2')])]),
                      ]),
                    ],
                  ),
                  BookImageBlock(

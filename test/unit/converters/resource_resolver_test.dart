...
void main() {
  group('ResourceResolver & ResourceRequestsCollector Tests', () {
    test(
      'collectResourceRequestsFromBook extracts all image requests from rich AST',
      () {
        final book = Book(
                ...
                BookParagraph(
                  inlines: [
                    ...
                    BookImageInline(
                      ...
                      attributes: {
                        'source-src': 'https://example.com/annot.png',
                      },
                    ),
                  ],
                ),
                              ...
                              BookParagraph(
                                inlines: [
                                  BookLink(
                                    ...
                                    children: const [
                                      BookImageInline(
                                        ref: BookResourceRef('table_cell.png'),
                                      ),
                                    ],
                                  ),
                                ],
                          ...
                          BookPoemLine(
                            inlines: [
                              BookStrong(
                                children: [
                                  BookImageInline(
                                    ref: BookResourceRef('poem_icon.png'),
                                  ),
                                ],
                              ),
                            ],
    ...
    test(
      'resolveResources: downloads pending resources with progress callbacks',
      () async {
        final book = Book(
          metadata: const BookMetadata(
            id: 'test-res',
    ...
    test(
      'resolveResources: handles failure and already resolved resources gracefully',
      () async {
        ...
        final allResolvedBook = await resolvedBook.resolveResources(
          (req, {onByteProgress}) async => null,
        );
        expect(
          allResolvedBook.resources.length,
          equals(resolvedBook.resources.length),
        );
      },
    );
  });

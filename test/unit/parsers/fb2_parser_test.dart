  ...
  group('Fb2Parser Unit Tests', () {
    ...
    group('1. Section Hierarchy and Arbitrary Depth', () {
      test('Parses multi-level nested sections with IDs and titles', () {
        ...
        expect((vol1.title.first as BookText).text, equals('Volume 1: Origins'));
        expect(vol1.blocks.length, equals(3)); // 1 paragraph + 2 child sections

        expect(vol1.blocks[0], isA<BookParagraph>());
        expect(((vol1.blocks[0] as BookParagraph).inlines.first as BookText).text, equals('Introductory paragraph of Volume 1.'));

        // Level 2: Chapter 1
        expect(vol1.blocks[1], isA<BookSection>());
        final ch1 = vol1.blocks[1] as BookSection;
        expect(ch1.id, equals('ch-1'));
        expect((ch1.title.first as BookText).text, equals('Chapter 1: The Gathering'));
        ...
        expect((sec11.title.first as BookText).text, equals('Section 1.1: Dawn'));
        expect(sec11.blocks.length, equals(1));
        expect(((sec11.blocks[0] as BookParagraph).inlines.first as BookText).text, equals('Deep nested text.'));

        // Level 2: Chapter 2
        expect(vol1.blocks[2], isA<BookSection>());
        final ch2 = vol1.blocks[2] as BookSection;
        expect(ch2.id, equals('ch-2'));
        expect((ch2.title.first as BookText).text, equals('Chapter 2: The Journey'));
      });
      ...
      test('Parses section without ID and subtitle/empty-line elements', () {
        ...
        expect((subtitle.text.first as BookText).text, equals('Subtitle of Section'));
    ...
    group('2. Epigraphs and Quotes with Poems & Authors', () {
      test('Parses <epigraph> with paragraphs, poems, and text-author', () {
        ...
        expect((epigraph.citation.first as BookText).text, equals('Robert Frost'));

        expect(epigraph.blocks.length, equals(2));
        expect(epigraph.blocks[0], isA<BookParagraph>());
        expect(((epigraph.blocks[0] as BookParagraph).inlines.first as BookText).text, equals('Knowledge is a journey, not a destination.'));
        ...
        expect((poem.stanzas[0].lines[0].inlines.first as BookText).text, equals('The woods are lovely, dark and deep,'));
        expect((poem.stanzas[0].lines[1].inlines.first as BookText).text, equals('But I have promises to keep.'));
      });

      test('Parses <cite> with multiple paragraphs and text-author without epigraph attribute', () {
        const xml = '''
          <cite>
        ...
        expect((quote.citation.first as BookText).text, equals('Immanuel Kant'));
      });
    });

    group('3. Footnotes and Links', () {
      test('Parses footnote links with type="note", #n_, #note prefixes into BookFootnoteRef', () {
        const xml = '''
          <p>
        ...
        expect((links.first.children.first as BookText).text, equals('Dart Website'));
      });
    });

    group('4. Custom Styles (<style name="...">)', () {
      test('Parses <style name="..."> into BookNamedStyle preserving nested inlines', () {
        const xml = '''
          <p>
        ...
        final style = p.inlines.firstWhere((i) => i is BookNamedStyle) as BookNamedStyle;
        ...
        expect(((style.inlines[1] as BookStrong).children.first as BookText).text, equals('bold'));
        expect((style.inlines[2] as BookText).text, equals(' text'));
      });

      test('Flattens <style> without name attribute without wrapping in BookNamedStyle', () {
        const xml = '<p>Text with <style><emphasis>anonymous style</emphasis></style> text.</p>';
        final blocks = parser.parseFromString(xml);
        final p = blocks.first as BookParagraph;
    ...
    group('5. Images and Tables with Attributes', () {
      test('Parses block and inline images with id, alt, title, and l:href / href attributes', () {
        const xml = '''
          <section>
      ...
      test('Uses custom BookResourceRegistrar if provided', () {
        ...
        expect((blocks.first as BookImageBlock).ref.id, equals('custom_mapped_#photo.jpg'));
      });

      test('Parses tables with colspan, rowspan, align, and valign attributes', () {
        const xml = '''
          <table>
            <tr>
    ...
    group('6. Code Blocks and Inline Formatting', () {
      test('Parses <code> and <pre> as BookCodeBlock', () {
        ...
        expect((blocksCode.first as BookCodeBlock).code, contains('print(42);'));
        ...
        expect((blocksPre.first as BookCodeBlock).code, equals('const pi = 3.14159;'));
      });

      test('Parses strong, emphasis, strikethrough/strike, sub, sup, and inline code', () {
        const xml = '''
          <p>
            <strong>Bold</strong>,
        ...
        expect(p.inlines.any((i) => i is BookCodeSpan && i.code == 'int x = 0;'), isTrue);
      });
    });
    ...
    group('7. Strict Mode & Logger Handling', () {
      ...
      test('Logs warning and parses children in non-strict mode for unhandled tag', () {
        ...
        const xml = '<custom-wrapper><p>Paragraph inside wrapper</p></custom-wrapper>';
        ...
        expect(((blocks.first as BookParagraph).inlines.first as BookText).text, equals('Paragraph inside wrapper'));
      });
    });

...
void main() {
  group('Comprehensive Branch Coverage & Edge Case Tests', () {
    test('DartBook facade handles resourceResolver and unknown formats/encoders', () async {
...
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0" xmlns:l="http://www.w3.org/1999/xlink">
  ...
  <body>
    <section>
      ...
      expect(loadedBook.metadata.srcTitleInfo?.title, equals('Original Title'));
      expect(loadedBook.metadata.publishInfo?.city, equals('Москва'));
      expect(loadedBook.metadata.primarySeries?.url, equals(Uri.parse('https://example.com/series/1')));

      expect(loadedBook.metadata.contributorsByRole(BookContributorRole.author), isNotEmpty);

    });

    test('BookConverter class instances (EpubConverter, Fb2Converter, Fb2ZipConverter)', () async {
      ...
      final dummyBook = const Book(
        metadata: BookMetadata(id: 'c1', title: 'Converter Book', language: 'ru'),
        content: BookContent(blocks: [BookParagraph(inlines: [BookText('Hi')])]),
        resources: [],
      );
    ...
    test('BookRegistry dynamic decoder and encoder registration', () {
      ...
      expect(BookRegistry.findDecoder(Uint8List.fromList([99, 99]), extension: 'dummy'), isNotNull);
    ...
    test('Exceptions toString() formatting', () {
      ...
      expect(ex2.toString(), contains('BookParseException: Parse failed (tag: <body>, line: 42)'));

      const ex3 = BookMalformedMetadataException('Bad metadata');
      expect(ex3.toString(), contains('BookMalformedMetadataException: Bad metadata'));
    });
                  ...
                  children: [
                    BookSuperscript(
                      children: [
                        BookSubscript(
                          children: [
                            BookImageInline(ref: BookResourceRef('nested_inline.png')),
                          ],
                        ),
                      ],
                    ),
    ...
    test('Fb2Encoder and EpubEncoder serialize Raw HTML and Raw XML inlines', () async {
      final rawBook = const Book(
        metadata: BookMetadata(id: 'raw-book', title: 'Raw Book', language: 'en'),
        content: BookContent(
          blocks: [
            BookParagraph(
              inlines: [
    ...
    test('ResourceNamingPolicy covers all extension deduction branches', () {
      ...
      expect(policy.generateName('http://site.com/pic.svg', isInline: false, index: 1), equals('pic.svg'));
      expect(policy.generateName('http://site.com/audio.mp3', isInline: false, index: 2), equals('audio.mp3'));
      expect(policy.generateName('http://site.com/video.mp4', isInline: false, index: 3), equals('video.mp4'));
      expect(policy.generateName('http://site.com/track.opus', isInline: false, index: 4), equals('track.opus'));
      expect(policy.generateName('http://site.com/font.woff2', isInline: false, index: 5), equals('font.woff2'));

      // Data URIs for diverse mime types
      expect(policy.generateName('data:image/gif;base64,R0lGOD...', isInline: false, index: 6), equals('img_006.gif'));
      expect(policy.generateName('data:image/webp;base64,UklGR...', isInline: false, index: 7), equals('img_007.webp'));
      expect(policy.generateName('data:image/svg+xml;base64,...', isInline: false, index: 8), equals('img_008.svg'));
      expect(policy.generateName('data:image/avif;base64,...', isInline: false, index: 9), equals('img_009.avif'));
      expect(policy.generateName('data:image/jxl;base64,...', isInline: false, index: 10), equals('img_010.jxl'));
      expect(policy.generateName('data:application/octet-stream;base64,...', isInline: false, index: 11), equals('img_011.png'));
    });

    test('Fb2Converter and Fb2ZipConverter helper static methods with options and resolvers', () async {
      final book = const Book(
        metadata: BookMetadata(id: 'z-book', title: 'Zip Book', language: 'ru'),
        content: BookContent(blocks: [BookParagraph(inlines: [BookText('Content')])]),
        resources: [],
      );

      final zip1 = await Fb2Converter.bookToFb2(book, isZip: true, resourceResolver: (req, {onByteProgress}) async => null);
      expect(zip1, isNotEmpty);

      final zip2 = await Fb2ZipConverter.bookToFb2Zip(book, resourceResolver: (req, {onByteProgress}) async => null);
    ...
    test('Fb2Decoder edge cases: middle-name, nickname only, flat footnotes, malformed base64', () {
      final fb2Xml = utf8.encode('''<?xml version="1.0" encoding="utf-8"?>
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  ...
  <binary id="b2.png" content-type="image/png">
    %%%invalid_base64%%%
      ...
      expect(book.metadata.contributors[0].name.display, equals('Лев Николаевич Толстой'));
    ...
    test('OcfContainer error handling and DRM paths parsing', () {
      ...
      expect(() => OcfContainer.fromArchive(emptyArchive), throwsA(isA<EpubInvalidPackageException>()));

      const emptyContainer = OcfContainer(rootfiles: []);
      expect(() => emptyContainer.primaryOpfPath, throwsA(isA<EpubInvalidPackageException>()));
...
<encryption xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <EncryptedData>
    ...
    <CipherData>
      ...
      encArchive.addFile(ArchiveFile('META-INF/encryption.xml', encBytes.length, encBytes));

      final drmPaths = OcfContainer.parseEncryptionPaths(encArchive);
      expect(drmPaths, contains('secret.xhtml'));
    });

    test('Fb2Decoder strictMode exceptions, logger integration, and canDecode ZIP check', () {
      ...
      expect(
        () => decoder.decode(Uint8List.fromList(utf8.encode(badSyntaxXml)), options: const BookDecodingOptions(strictMode: true)),
        throwsA(isA<BookParseException>()),
      );

      // Strict mode: Missing description element
      const noDescXml = '<?xml version="1.0"?><FictionBook><body><section><p>Hi</p></section></body></FictionBook>';
      expect(
        () => decoder.decode(Uint8List.fromList(utf8.encode(noDescXml)), options: const BookDecodingOptions(strictMode: true)),

...
void main() {
  group('Book Models Unit Tests', () {
    group('1. Freezed Models Immutability and copyWith()', () {
      test(
        'BookMetadata is immutable and copyWith creates a new instance with updated fields',
        () {
      ...
      test(
        'BookPublishInfo is immutable and copyWith updates fields correctly',
        () {
      ...
      test(
        'BookSourceTitleInfo is immutable and copyWith updates fields correctly',
        () {
      ...
      test('BookSeries is immutable and copyWith updates number and URL', () {
        const originalSeries = BookSeries(name: 'Властелин Колец', number: 1);
        ...
        expect(
          updatedSeries.url,
          equals(Uri.parse('https://lotr.fandom.com/wiki/The_Two_Towers')),
        );
      });

      test(
        'Book root model is immutable and copyWith updates metadata and content',
        () {
    ...
    group('2. Deep Value Equality and HashCode', () {
      test(
        'Identical BookMetadata instances evaluate to equal with identical hashCode',
        () {
    ...
    group('3. Model Helper Methods and Logic', () {
      test('BookMetadata primarySeries returns the first series or null', () {
        ...
        const metaWithoutSeries = BookMetadata(
          id: 'm2',
          title: 'T',
          language: 'ru',
        );
      ...
      test('BookMetadata contributorsByRole filters contributors by role', () {
        ...
        final authors = meta
            .contributorsByRole(BookContributorRole.author)
            .toList();
        ...
        final translators = meta
            .contributorsByRole(BookContributorRole.translator)
            .toList();
        expect(translators.length, equals(1));
        expect(translators.first.name.first, equals('Translator'));

        final illustrators = meta
            .contributorsByRole(BookContributorRole.illustrator)
            .toList();
        expect(illustrators, isEmpty);
      });

      test(
        'PersonName toDisplayString formats display string across variations',
        () {
    ...
    group('4. BookBuilder Fluent Interface and Step-by-Step Construction', () {
      test('Builds complete book step-by-step using builder methods', () async {
        final builder = BookBuilder(
          ...
          genres: const [
            BookGenre(code: 'sf_space', name: 'Космическая фантастика'),
          ],
        ...
        // 1. Set cover
        builder.setCover(
          const BookResourceRef('cover.png'),
          alt: 'Обложка книги',
        );

        // 2. Set annotation HTML
        builder.setAnnotationHtml(
          '<p>Захватывающая история о покорении <strong>дальних миров</strong>.</p>',
        );
        ...
        expect(
          ...
          equals(Uri.parse('https://example.com/books/galaxy-saga-1')),
        );
        ...
        expect(
          (sec2.title.first as BookText).text,
          equals('Глава 2: Гиперпрыжок'),
        );
      ...
      test('Supports custom and sequential resource naming policies', () async {
        ...
        await sequentialBuilder.addChapterHtml(
          '<p><img src="https://cdn.site.com/assets/photos/pic1.jpeg"/></p>',
        );
      ...
      test(
        'Resolves pending resources asynchronously with custom resource resolver',
        () async {
          ...
          expect(
            resolvedRequests.first.source,
            equals('https://images.site.com/hero.png'),
          );
          expect(book.resources.length, equals(1));
          expect(
            book.resources.first.bytes,

    ...
    test(
      ...
      () async {
        ...
        expect(
          file.existsSync(),
          isTrue,
          reason: 'litres_sample.fb2 fixture must exist',
        );

        final rawBytes = file.readAsBytesSync();
        final book = await DartBook.load(
          rawBytes,
          filename: 'litres_sample.fb2',
        );
        ...
        expect(
          ...
          containsAll(['prose_classic', 'poetry']),
        );
        ...
        final rootEpigraph =
            blocks.firstWhere(
                  (b) =>
                      b is BookQuote && b.attributes['fb2-type'] == 'epigraph',
                )
                as BookQuote;
        expect(rootEpigraph.citation.isNotEmpty, isTrue);
        expect(
          (rootEpigraph.citation.first as BookText).text,
          contains('Вяземский'),
        );

        // Main section
        final section =
            blocks.firstWhere((b) => b is BookSection) as BookSection;
        expect((section.title.first as BookText).text, equals('Глава I'));

        // Section epigraph
        final secEpigraph =
            section.blocks.firstWhere(
                  (b) =>
                      b is BookQuote && b.attributes['fb2-type'] == 'epigraph',
                )
                as BookQuote;
        expect(
          (secEpigraph.citation.first as BookText).text,
          contains('Грибоедов'),
        );

        // Poem
        final poem =
            section.blocks.firstWhere((b) => b is BookPoem) as BookPoem;
        expect(poem.stanzas.length, equals(1));
        expect(poem.stanzas.first.lines.length, equals(4));
        expect(
          (poem.stanzas.first.lines.first.inlines.first as BookText).text,
          contains('Так думал молодой повеса'),
        );

        // Footnote references in paragraphs
        final paragraphs = section.blocks.whereType<BookParagraph>().toList();
        final hasFnRef1 = paragraphs.any(
          (p) => p.inlines.any((i) => i is BookFootnoteRef && i.id == 'note_1'),
        );
        final hasFnRef2 = paragraphs.any(
          (p) => p.inlines.any((i) => i is BookFootnoteRef && i.id == 'note_2'),
        );
        ...
        expect(
          redecoded.content.footnotes.length,
          equals(book.content.footnotes.length),
        );
        GoldenComparator.assertContentEquals(
          redecoded.content,
          book.content,
          context: 'LitRes FB2 Roundtrip',
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(
          file.existsSync(),
          isTrue,
          reason: 'fb2_21_sample.fb2 fixture must exist',
        );

        final rawBytes = file.readAsBytesSync();
        final book = await DartBook.load(
          rawBytes,
          filename: 'fb2_21_sample.fb2',
        );
        ...
        expect(
          book.metadata.srcTitleInfo!.title,
        ...
        expect(
          book.metadata.srcTitleInfo!.authors.first.name.display,
          equals('Alan Mathison Turing'),
        );

        // 2. Verify Key AST Nodes
        final blocks = book.content.blocks;
        expect(
          blocks.any((b) => b is BookHeading && b.level == 2),
          isTrue,
        ); // subtitle
        ...
        final codeSpan =
            p3.inlines.firstWhere((i) => i is BookCodeSpan) as BookCodeSpan;
        ...
        final inlineImg =
            p4.inlines.firstWhere((i) => i is BookImageInline)
                as BookImageInline;
        ...
        final table =
            sec2.blocks.firstWhere((b) => b is BookTable) as BookTable;
        ...
        final quote =
            sec2.blocks.firstWhere((b) => b is BookQuote) as BookQuote;
        expect((quote.citation.first as BookText).text, equals('Donald Knuth'));

        final blockImg =
            sec2.blocks.firstWhere((b) => b is BookImageBlock)
                as BookImageBlock;
    ...
    test(
      ...
      () async {
        ...
        expect(
          file.existsSync(),
          isTrue,
          reason: 'fb2_22_sample.fb2 fixture must exist',
        );

        final rawBytes = file.readAsBytesSync();
        final book = await DartBook.load(
          rawBytes,
          filename: 'fb2_22_sample.fb2',
        );
        ...
        expect(
          ...
          equals(Uri.parse('https://cybernetics.example.org/series/cyberiada')),
        );
        expect(
          book.metadata.keywords,
          containsAll(['наука', 'кибернетика', 'будущее']),
        );
        ...
        expect(
          (namedStyles[0].inlines.first as BookText).text,
          equals('критически важный термин'),
        );
        expect(namedStyles[1].name, equals('term'));
        expect(
          (namedStyles[1].inlines.first as BookText).text,
          equals('конструкт'),
        );

        final p2 = sec1.blocks[1] as BookParagraph;
        final strong =
            p2.inlines.firstWhere((i) => i is BookStrong) as BookStrong;
        expect(
          strong.children.any(
            (c) => c is BookNamedStyle && c.name == 'code-style',
          ),
          isTrue,
        );

        // Verify multiple bodies (commentary body was parsed into blocks)
        final allSecTitles = blocks
            .whereType<BookSection>()
            .map((s) => (s.title.first as BookText).text)
            .toList();

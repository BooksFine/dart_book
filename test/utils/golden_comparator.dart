...
class GoldenComparator {
  ...
  static Map<String, dynamic> contentToJson(
    BookContent content, {
    bool normalize = true,
  }) {
  ...
  static Map<String, dynamic> blockToJson(BookBlock block) {
    return switch (block) {
            ...
            .map(
              (r) => {
                'cells': r.cells
                    .map(
                      (c) => {
                        if (c.colSpan != null && c.colSpan! > 1)
                          'colSpan': c.colSpan,
                        if (c.rowSpan != null && c.rowSpan! > 1)
                          'rowSpan': c.rowSpan,
        ...
        'stanzas': p.stanzas
            .map(
              (s) => {
                'lines': s.lines
                    .map(
                      (l) => {'inlines': l.inlines.map(inlineToJson).toList()},
                    )
                    .toList(),
              },
            )
      ...
      BookAudioBlock a => {'type': 'audio_block', 'ref': a.ref.id},
      BookVideoBlock v => {'type': 'video_block', 'ref': v.ref.id},
      BookMathBlock m => {'type': 'math_block', 'mathml': m.mathml},
      BookSvgBlock svg => {'type': 'svg_block', 'svg': svg.svg},
  ...
  static Map<String, dynamic> inlineToJson(BookInline inline) {
    return switch (inline) {
      ...
      BookFootnoteRef fn => {
        'type': 'footnote_ref',
        'id': fn.id,
        if (fn.label.isNotEmpty) 'label': fn.label.map(inlineToJson).toList(),
      },
      BookSuperscript sup => {
        'type': 'superscript',
  ...
  static void assertBookMatchesGoldenFile(Book book, String goldenFilePath) {
    ...
    if (shouldUpdate || !file.existsSync()) {
      ...
      if (!shouldUpdate) {
        fail(
          'Golden file was missing and has been created at: $goldenFilePath. Please re-run test.',
        );
      }
      return;
    }
  ...
  static void assertContentMatchesGoldenFile(
    BookContent content,
  ...
  static void assertContentEquals(
    BookContent actual,
    BookContent expected, {
    String? context,
  }) {

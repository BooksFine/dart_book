...
void assertBlockEquals(
  ...
  if (expected is! BookRawHtmlBlock && expected is! BookRawXmlBlock) {
    expect(
      ...
      reason:
          '$path: блок ${expected.runtimeType} не должен превращаться в BookRawHtmlBlock',
    );
    expect(
      actual,
      isNot(isA<BookRawXmlBlock>()),
      reason:
          '$path: блок ${expected.runtimeType} не должен превращаться в BookRawXmlBlock',
    );
  }
  ...
  switch ((actual, expected)) {
    ...
    case (BookHeading a, BookHeading e):
      expect(
        a.level,
        equals(e.level),
        reason: '$path: уровень заголовка должен совпадать',
      );
    ...
    case (BookList a, BookList e):
      expect(
        a.ordered,
        equals(e.ordered),
        reason: '$path: тип списка (ordered) должен совпадать',
      );
      expect(
        a.items.length,
        equals(e.items.length),
    ...
    case (BookTable a, BookTable e):
      expect(
        a.rows.length,
        equals(e.rows.length),
        reason: '$path: число строк таблицы должно совпадать',
      );
      for (var r = 0; r < a.rows.length; r++) {
        ...
        for (var c = 0; c < a.rows[r].cells.length; c++) {
          ...
          expect(
            ac.colSpan,
            equals(ec.colSpan),
            reason: '$path.rows[$r].cells[$c].colSpan',
          );
          expect(
            ac.rowSpan,
            equals(ec.rowSpan),
            reason: '$path.rows[$r].cells[$c].rowSpan',
          );
          expect(
            ac.align,
            equals(ec.align),
            reason: '$path.rows[$r].cells[$c].align',
          );
          expect(
            ac.vAlign,
            equals(ec.vAlign),
    ...
    case (BookPoem a, BookPoem e):
      expect(
        a.stanzas.length,
        equals(e.stanzas.length),
        reason: '$path: число строф должно совпадать',
      );
      for (var s = 0; s < a.stanzas.length; s++) {
        ...
        for (var l = 0; l < a.stanzas[s].lines.length; l++) {
          assertInlineListEquals(
            a.stanzas[s].lines[l].inlines,
            e.stanzas[s].lines[l].inlines,
            path: '$path.stanza[$s].line[$l]',
          );
        }
      }
    ...
    case (BookImageBlock a, BookImageBlock e):
      expect(
        a.ref.id.endsWith(e.ref.id),
        isTrue,
        reason: '$path: id ресурса изображения должен совпадать',
      );
      expect(
        a.id,
        equals(e.id),
        reason: '$path: id изображения должен совпадать',
      );
      expect(
        a.alt,
        equals(e.alt),
        reason: '$path: alt изображения должен совпадать',
      );
      expect(
        a.title,
        equals(e.title),
        reason: '$path: title изображения должен совпадать',
      );
    ...
    case (BookMathBlock a, BookMathBlock e):
      expect(
        a.mathml,
        equals(e.mathml),
        reason: '$path: формулы MathML должны совпадать',
      );
    ...
    case (BookSvgBlock a, BookSvgBlock e):
      expect(
        a.svg,
        equals(e.svg),
        reason: '$path: SVG графика должна совпадать',
      );
...
void assertInlineListEquals(
  List<BookInline> actual,
  List<BookInline> expected, {
  required String path,
}) {
  expect(
    actual.length,
    equals(expected.length),
    reason: '$path: число инлайновых элементов должно совпадать',
  );
  for (var i = 0; i < actual.length; i++) {
...
void assertInlineEquals(
  ...
  switch ((actual, expected)) {
    ...
    case (BookLink a, BookLink e):
      expect(
        a.href.toString(),
        equals(e.href.toString()),
        reason: '$path: ссылка href должна совпадать',
      );
      assertInlineListEquals(a.children, e.children, path: '$path.children');
    ...
    case (BookImageInline a, BookImageInline e):
      expect(
        a.ref.id.endsWith(e.ref.id),
        isTrue,
        reason: '$path: id встроенного изображения должен совпадать',
      );
      expect(
        a.id,
        equals(e.id),
        reason: '$path: id инлайн-изображения должен совпадать',
      );
      expect(
        a.alt,
        equals(e.alt),
        reason: '$path: alt инлайн-изображения должен совпадать',
      );
      expect(
        a.title,
        equals(e.title),
        reason: '$path: title инлайн-изображения должен совпадать',
      );

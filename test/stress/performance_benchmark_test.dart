  ...
  group('Stress & Performance Benchmark Tests', () {
    test('HtmlParser: Measures parser performance on 10,000 paragraphs', () {
      ...
      for (var i = 0; i < 10000; i++) {
        buffer.write(
          '<p>Параграф №$i с <strong>жирным</strong> и <em>курсивным</em> текстом, <a href="#p$i">ссылкой</a> и <code>кодом</code>.</p>',
        );
      }
      buffer.write('</div>');
      ...
      print(
        '[BENCHMARK] HtmlParser 10,000 paragraphs: ${stopwatch.elapsedMilliseconds} ms (${blocks.length} blocks parsed)',
      );
    ...
    test('Fb2Decoder: Measures parsing performance on 5,000 sections', () {
      ...
      buffer.write(
        '<description><title-info><book-title>Большая FB2 Книга</book-title></title-info></description>',
      );
      buffer.write('<body>');
      for (var i = 0; i < 5000; i++) {
        buffer.write(
          '<section><title><p>Глава $i</p></title><p>Текст главы $i с <emphasis>выделением</emphasis> и <a type="note" l:href="#n$i">[сноской]</a>.</p></section>',
        );
      }
      buffer.write('</body>');
      ...
      print(
        '[BENCHMARK] Fb2Decoder 5,000 sections: ${stopwatch.elapsedMilliseconds} ms (book.metadata.id: ${book.metadata.id})',
      );
    ...
    test(
      'War & Peace Scale: Measures Roundtrip conversion performance on 1,000 chapters and 10,000 paragraphs',
      () async {
        ...
        stopFb2Enc.stop();
        print(
          '[BENCHMARK] Fb2Encoder 1,000 chapters (10,000 paragraphs): ${stopFb2Enc.elapsedMilliseconds} ms (${fb2Bytes.length} bytes)',
        );
        ...
        stopFb2Dec.stop();
        print(
          '[BENCHMARK] Fb2Decoder 1,000 chapters (10,000 paragraphs): ${stopFb2Dec.elapsedMilliseconds} ms',
        );
        ...
        stopEpubEnc.stop();
        print(
          '[BENCHMARK] EpubEncoder 1,000 chapters (10,000 paragraphs): ${stopEpubEnc.elapsedMilliseconds} ms (${epubBytes.length} bytes)',
        );
        ...
        stopEpubDec.stop();
        print(
          '[BENCHMARK] EpubDecoder 1,000 chapters (10,000 paragraphs): ${stopEpubDec.elapsedMilliseconds} ms',
        );
    ...
    test(
      'Isolates: Verifies 144 FPS non-blocking execution with DartBook.loadIsolated and DartBook.encodeIsolated',
      () async {
        final builder = BookBuilder(title: 'Изолированная книга 144 FPS');
        for (var i = 0; i < 50; i++) {
          await builder.addChapterHtml(
            '<p>Параграф $i для изолята</p>',
            title: 'Глава $i',
          );
        }
        final book = await builder.build();
        ...
        // 2. loadIsolated (FB2)
        final loadedFb2 = await DartBook.loadIsolated(
          fb2Bytes,
          filename: 'book.fb2',
        );
        ...
        // 4. loadIsolated (EPUB)
        final loadedEpub = await DartBook.loadIsolated(
          epubBytes,
          filename: 'book.epub',
        );
        expect(
          loadedEpub.metadata.title,
          equals('Изолированная книга 144 FPS'),
        );

...
void main() {
  group('Stress: Corrupted XML & Error Recovery', () {
    test(
      'FB2: Strips invalid XML 1.0 control characters (0x00-0x1F) in non-strict mode',
      () {
        final xmlWithControls =
        ...
        final book = Fb2Decoder().decode(
          bytes,
          options: const BookDecodingOptions(strictMode: false),
        );
    ...
    test(
      'FB2: Recovers from unescaped ampersands (AT&T, Barnes & Noble, R&D) in non-strict mode',
      () {
...
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  ...
  <body>
    ...
    test(
      'FB2: Sanitizes undeclared HTML entities (&nbsp;, &mdash;, &laquo;, &raquo;) in non-strict mode',
      () {
...
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  ...
  <body>
    ...
    test(
      'FB2: Resilient Base64 decoding in <binary> handles whitespace, newlines, and missing padding',
      () {
...
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  ...
  <binary id="img_empty" content-type="image/png"></binary>
        ...
        final resSpaces = book.resources.firstWhere(
          (r) => r.id == 'img_spaces',
        );
    ...
    test(
      'FB2: Automatic encoding fallback when Windows-1251 bytes are paired with UTF-8 XML header',
      () {
        // Create XML with <?xml version="1.0" encoding="UTF-8"?> header, but actual bytes in Windows-1251
        final header = utf8.encode(
          '<?xml version="1.0" encoding="UTF-8"?><FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0"><description><title-info><book-title>',
        );
        ...
        final body = utf8.encode(
          '</book-title></title-info></description><body><p>Текст</p></body></FictionBook>',
        );
  ...
  group('Stress: Strict Mode vs Lenient Fallback', () {
    test(
      'HtmlParser: strictMode = true throws BookParseException with tag name; strictMode = false falls back',
      () {
        const htmlSrc =
            '<p>Good paragraph</p><unknown-widget data="val">Widget Text</unknown-widget>';
          ...
          throwsA(
            isA<BookParseException>()
                ...
                .having(
                  (e) => e.message,
                  'message',
                  contains('Unhandled HTML element'),
                ),
          ),
        );
    ...
    test(
      'HtmlParser: strictMode = true throws BookParseException on unhandled inline elements',
      () {
        const htmlSrc =
            '<p>Normal text <custom-inline>Custom</custom-inline> end</p>';
    ...
    test(
      'Fb2Parser: strictMode = true throws BookParseException on unhandled FB2 tags',
      () {
        const fb2Section =
            '<section><title><p>Title</p></title><custom-fb2-tag>Text</custom-fb2-tag></section>';
    ...
    test(
      'Fb2Decoder & EpubDecoder: strictMode = true throws BookMalformedMetadataException on missing title',
      () async {
        // FB2 missing title
...
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  ...
  <body><p>Text</p></body>
        ...
        final fb2Book = Fb2Decoder().decode(
          fb2Bytes,
          options: const BookDecodingOptions(strictMode: false),
        );

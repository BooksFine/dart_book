...
void main() {
  group('Security: ZIP Slip & Path Traversal Protection', () {
    test(
      'EPUB: Traversal paths in container.xml full-path are safely handled without escaping',
      () async {
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    <rootfile full-path="../../etc/passwd" media-type="application/oebps-package+xml"/>
        ...
        archive.addFile(
          ArchiveFile('META-INF/container.xml', contBytes.length, contBytes),
        );
...
<package xmlns="http://www.idpf.org/2007/opf" unique-identifier="pub-id" version="3.0">
  ...
  <spine>
    <itemref idref="chap1"/>
        ...
        archive.addFile(
          ArchiveFile('../../etc/passwd', opfBytes.length, opfBytes),
        );

        const chapHtml = '<html><body><p>Safe content</p></body></html>';
        final chapBytes = utf8.encode(chapHtml);
        archive.addFile(
          ArchiveFile('chap1.xhtml', chapBytes.length, chapBytes),
        );
    ...
    test(
      'EPUB: Manifest items with ../../ and Windows ..\\..\\ traversal paths do not escape sandbox',
      () async {
        final archive = Archive();
        final mimeBytes = utf8.encode('application/epub+zip');
        archive.addFile(
...
<package xmlns="http://www.idpf.org/2007/opf" unique-identifier="pub-id" version="3.0">
  ...
  <spine>
    ...
    <itemref idref="chap3"/>
        ...
        const chap1Html =
            '<html><body><p>Chapter 1 sanitized</p></body></html>';
        final c1Bytes = utf8.encode(chap1Html);
        archive.addFile(ArchiveFile('evil.xhtml', c1Bytes.length, c1Bytes));

        const chap2Html =
            '<html><body><p>Chapter 2 sanitized</p></body></html>';
        final c2Bytes = utf8.encode(chap2Html);
        archive.addFile(
          ArchiveFile('Windows/cmd.xhtml', c2Bytes.length, c2Bytes),
        );

        const chap3Html =
            '<html><body><p>Chapter 3 sanitized</p></body></html>';
        final c3Bytes = utf8.encode(chap3Html);
        archive.addFile(
          ArchiveFile('OEBPS/etc/passwd', c3Bytes.length, c3Bytes),
        );
    ...
    test(
      'EPUB: Resource references with traversal paths stay safely scoped',
      () async {
        final archive = Archive();
        final mimeBytes = utf8.encode('application/epub+zip');
        archive.addFile(
...
<package xmlns="http://www.idpf.org/2007/opf" unique-identifier="pub-id" version="3.0">
  ...
  <spine>
    <itemref idref="chap1"/>
        ...
        const chapHtml =
            '<html><body><img src="../../images/secret.png"/><img src="..\\..\\Windows\\logo.png"/></body></html>';
        archive.addFile(
          ArchiveFile(
            'OEBPS/chap1.xhtml',
            utf8.encode(chapHtml).length,
            utf8.encode(chapHtml),
          ),
        );

        final imgBytes = Uint8List.fromList([137, 80, 78, 71]);
        archive.addFile(
          ArchiveFile('images/secret.png', imgBytes.length, imgBytes),
        );
        archive.addFile(
          ArchiveFile('Windows/logo.png', imgBytes.length, imgBytes),
        );
        ...
        expect(
          book.resources.map((r) => r.id),
          containsAll(['epub-res-img1', 'epub-res-img2']),
        );
      },
    );

    test(
      'FB2.ZIP: Archive containing traversal entry names (../../evil.fb2, ..\\..\\cmd.exe) decodes safely in-memory',
      () {
...
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">
  ...
  <body>
    <p>Безопасный текст в памяти</p>
        ...
        archive1.addFile(
          ArchiveFile('../../evil_dir/book.fb2', xmlBytes.length, xmlBytes),
        );
        ...
        archive3.addFile(
          ArchiveFile('/etc/shadow/book.xml', xmlBytes.length, xmlBytes),
        );
  ...
  group('Security: Decompression Attacks & Resource Quotas', () {
    test(
      'Handles highly compressed / large zero-filled archive payload gracefully',
      () async {
        ...
        largeBuffer.write(
          '<description><title-info><book-title>Decompression Test</book-title><lang>ru</lang></title-info></description>',
        );
        largeBuffer.write('<body>');
        for (var i = 0; i < 2000; i++) {
          largeBuffer.write(
            '<p>Повторяющийся длинный параграф для проверки компрессии №$i</p>',
          );
        }
        largeBuffer.write('</body>');
    ...
    test(
      'Rejects corrupted, truncated and malformed ZIP streams with BookFormatException',
      () {
        ...
        expect(
          () => zipDecoder.decode(
            Uint8List.fromList([0x50, 0x4B, 0x03, 0x04, 0xFF, 0xFE, 0xFD]),
          ),
          throwsA(isA<BookFormatException>()),
        );
    ...
    test(
      'Handles ZIP archive with large number of metadata entries without memory exhaustion',
      () async {
        final archive = Archive();
        final mimeBytes = utf8.encode('application/epub+zip');
        archive.addFile(
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    <rootfile full-path="OEBPS/content.opf" media-type="application/oebps-package+xml"/>
        ...
        opfBuf.writeln(
          '  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:identifier id="pub-id">many-entries</dc:identifier><dc:title>Many Entries</dc:title><dc:language>en</dc:language></metadata>',
        );
        ...
        for (var i = 0; i < 200; i++) {
          archive.addFile(
            ArchiveFile('OEBPS/c$i.xhtml', chapBytes.length, chapBytes),
          );
        }
